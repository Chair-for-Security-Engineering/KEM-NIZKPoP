...
//   print_results("indcpa_dec: ", t, NTESTS);
  ...
  print_results("kyber_keypair_zkpop: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    if ((rv = crypto_kem_keypair_zkpop(pk, sk, &zkpop, &zkpop_size)) != 0)
    {
      printf("bad kyber_keypair_zkpop gen: %d\n", rv);
      return -1;
    }
    if ((rv = crypto_zkpop_verify(pk, zkpop, zkpop_size)) == 0)
    {
      free(zkpop);
    } else {
      printf("bad kyber_keypair_zkpop verify: %d\n", rv);
      return -1;
    }
  }
