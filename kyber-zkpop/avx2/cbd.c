...
{
#if KYBER_ETA1 == 2
  ALIGNED_UINT8(2*(ZKPOP_M+127)/128) buf;
  unsigned int i;
  __m256i f0, f1, f2, f3;
  const __m256i mask55 = _mm256_set1_epi32(0x55555555);
  const __m256i mask33 = _mm256_set1_epi32(0x33333333);
  ...
  prf(&buf.coeffs[0], 2*(ZKPOP_M+127)/128, seed, 0);
  for(i = 0; i < (ZKPOP_M+63)/64; i++) {
    f0 = _mm256_load_si256(&buf.vec[i]);
  ...
  prf(buf, 3*(ZKPOP_M+3)/4+8, seed, 0);
  for(i = 0; i < (ZKPOP_M+31)/32; i++) {
    f0 = _mm256_loadu_si256((__m256i *)&buf[24*i]);
    f0 = _mm256_permute4x64_epi64(f0,0x94);
    f0 = _mm256_shuffle_epi8(f0,shufbidx);
